...
self.onmessage = (event: MessageEvent<WorkerInput>) => {
  try {
    ...
    // Transfer the blocks buffer for zero-copy. The file compiles under the
    // DOM lib, where `self.postMessage` resolves to the Window overload — cast
    // to the worker-scope signature.
    (self as unknown as Worker).postMessage(result, [result.blocks.buffer as ArrayBuffer]);
  } catch (err) {
    self.postMessage({
      type: 'error',
      message: String(err),
