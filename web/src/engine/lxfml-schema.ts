...
export const LXFML_ATTRIBUTES: Readonly<Record<string, LxfmlSpec>> = {
  ...
  'Brick@itemNos': e('appearance', true, 'LEGO element numbers. For a HEAD the element names its print: `ldd-print-map.json` resolves it to the printed LDraw head (`printedHeadFor`, 2026-09-24)'),
  'Brick@version': e('metadata', false, 'mould version'),
  'Brick@decorationBriefId': e('appearance', true, 'decoration brief for the whole brick (3,751 sets). Read for HEADS only: its id is the print table\'s fallback key when the element is unknown (`decorationIdOf`)'),
  ...
  'Part@decoration': e('appearance', true, 'the print on this part (3,874 sets). Read for HEADS only (its leading id, when the brick has no decorationBriefId): a head resolves to its printed LDraw part. Every other print is still not carried'),
