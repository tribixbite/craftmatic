// Stateful AES-256-CFB8 for runtimes whose crypto API exposes AES but not the
// CFB8 mode used by Minecraft. AES block layout and key expansion follow
// FIPS-197; CFB8 shifts one ciphertext byte into the 16-byte register.
const S = new Uint8Array([
  99,124,119,123,242,107,111,197,48,1,103,43,254,215,171,118,202,130,201,125,250,89,71,240,173,212,162,175,156,164,114,192,
  183,253,147,38,54,63,247,204,52,165,229,241,113,216,49,21,4,199,35,195,24,150,5,154,7,18,128,226,235,39,178,117,
  9,131,44,26,27,110,90,160,82,59,214,179,41,227,47,132,83,209,0,237,32,252,177,91,106,203,190,57,74,76,88,207,
  208,239,170,251,67,77,51,133,69,249,2,127,80,60,159,168,81,163,64,143,146,157,56,245,188,182,218,33,16,255,243,210,
  205,12,19,236,95,151,68,23,196,167,126,61,100,93,25,115,96,129,79,220,34,42,144,136,70,238,184,20,222,94,11,219,
  224,50,58,10,73,6,36,92,194,211,172,98,145,149,228,121,231,200,55,109,141,213,78,169,108,86,244,234,101,122,174,8,
  186,120,37,46,28,166,180,198,232,221,116,31,75,189,139,138,112,62,181,102,72,3,246,14,97,53,87,185,134,193,29,158,
  225,248,152,17,105,217,142,148,155,30,135,233,206,85,40,223,140,161,137,13,191,230,66,104,65,153,45,15,176,84,187,22,
]);
const RCON = new Uint8Array([0,1,2,4,8,16,32,64,128,27,54]);

function expandKey(key) {
  if (!(key instanceof Uint8Array) || key.length !== 32) throw new Error('AES-256 key must be 32 bytes');
  const out = new Uint8Array(240);
  out.set(key);
  const temp = new Uint8Array(4);
  let at = 32;
  let rcon = 1;
  while (at < out.length) {
    temp.set(out.subarray(at - 4, at));
    if (at % 32 === 0) {
      const first = temp[0];
      temp[0] = S[temp[1]] ^ RCON[rcon++];
      temp[1] = S[temp[2]];
      temp[2] = S[temp[3]];
      temp[3] = S[first];
    } else if (at % 32 === 16) {
      for (let i = 0; i < 4; i++) temp[i] = S[temp[i]];
    }
    for (let i = 0; i < 4; i++, at++) out[at] = out[at - 32] ^ temp[i];
  }
  return out;
}

const xtime = value => ((value << 1) ^ (value & 0x80 ? 0x11b : 0)) & 255;

function encryptBlock(input, keys) {
  let state = Uint8Array.from(input, (value, i) => value ^ keys[i]);
  for (let round = 1; round <= 14; round++) {
    const shifted = new Uint8Array(16);
    for (let column = 0; column < 4; column++) {
      for (let row = 0; row < 4; row++) shifted[column * 4 + row] = S[state[((column + row) & 3) * 4 + row]];
    }
    state = shifted;
    if (round !== 14) {
      for (let column = 0; column < 4; column++) {
        const i = column * 4;
        const a = state[i], b = state[i + 1], c = state[i + 2], d = state[i + 3];
        const all = a ^ b ^ c ^ d;
        state[i] ^= all ^ xtime(a ^ b);
        state[i + 1] ^= all ^ xtime(b ^ c);
        state[i + 2] ^= all ^ xtime(c ^ d);
        state[i + 3] ^= all ^ xtime(d ^ a);
      }
    }
    const keyAt = round * 16;
    for (let i = 0; i < 16; i++) state[i] ^= keys[keyAt + i];
  }
  return state;
}

export class Aes256Cfb8 {
  constructor(key, iv, decrypt = false) {
    if (!(iv instanceof Uint8Array) || iv.length !== 16) throw new Error('AES-CFB8 IV must be 16 bytes');
    this.keys = expandKey(key);
    this.register = Uint8Array.from(iv);
    this.decrypt = decrypt;
  }

  update(input) {
    const source = input instanceof Uint8Array ? input : new Uint8Array(input);
    const output = new Uint8Array(source.length);
    for (let i = 0; i < source.length; i++) {
      const value = source[i] ^ encryptBlock(this.register, this.keys)[0];
      output[i] = value;
      this.register.copyWithin(0, 1);
      this.register[15] = this.decrypt ? source[i] : value;
    }
    return output;
  }
}
