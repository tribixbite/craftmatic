...
Select the wand in your hotbar to open its menu. Switch to another slot and back to reopen it. Pin the model's bottom corner, enter XYZ coordinates, and rotate in 90-degree steps. **View preview in world** closes the menu and shows a miniature point cloud in front of the player, with a direction marker and a full-size boundary near the pin; it does not place blocks. The miniature fits within eight blocks, stays above the player's feet, and refreshes every 12 game ticks. The particle budget is capped at 360 per refresh. Reopen the wand for **Place**, which separately confirms replacement of blocks in the target area. Structures and interactive entities use the same rotated coordinate frame.
...
The current Android QA uses the separate `My World` on the retail Pixel, preserving the original `test` world. A controlled version-only import proved that the oversized timestamp version caused `Unknown Pack Name`; the compact-version add-on imported successfully. Native tests confirmed hotbar activation, pinning, XYZ editing, 90-degree rotation, and particles without the menu reopening. The original full-size particle preview was too sparse for the 327-block model, motivating the nearby miniature. The earlier loading implementation stalled after 15 of 18 tiles, and its pending promise prevented cancellation; a completed placement/Undo cycle must be retested with the bounded loader before being claimed as verified. That partial build remains only in the remote QA area.
