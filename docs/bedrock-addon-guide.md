...
For a head with no LDraw print the table writes an IDENTITY name, Studio's
BL-copy form `3626cpb<N>.dat` (3,196 `n:` rows). No library ships it, so
every reader strips `pb<N>` and draws plain `3626c` exactly as before, but the
source now says which head it is. `scripts/gen-face-art.py <dir> <model>...`
...
**Publication:** clego `dbix_print_heads.py` (patch mode) renamed 4,005 head
lines in 1,196 published DbixConvV3 files (1,736 printed, 2,269 identity;
2,809 matched verbatim, 1,196 by order where an older run wrote other
numbers, 0 missed), and 76417 was regenerated by its recipe
(`_lxfml_assemble.ts --root-step` + `_lxfml_to_ldr.ts`) - byte-identical to
the published `d3a02437401c` apart from its 15 head names. Files and
sha256/12 in `output/faces-0924/PUBLISH.md`.
...
them bare, e.g. `101267`, with no mould prefix to form an identity name -
