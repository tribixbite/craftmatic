...
- Other importers: `bff-loader.ts` (BrickLink inventory → flat layout), `studio-colors.ts`, `ldd-colors.ts`.
  ...
  - `lxf-parser.ts` (.lxf/LDD) — **TWO alignment tables, Studio's `ldraw.xml` FIRST (2026-09-17), measured second.** `parseBoneTransform` (LXFML column-major→row-major) and `axisAngleToMatrix` are unchanged.
    ...
    - **FOURTH case: the ELEMENT fallback** (2026-09-24, port of clego `reconvert_dbix.Resolver.resolve_element`). A design NEITHER table names used to be drawn as `<designID>.dat` at identity — a missing part whenever the LDD number is not an LDraw file, which is exactly the doll heads above (LDD 28650 is LDraw 92198). A one-part brick's `itemNos` element ids now resolve through `/ldd-element-map.json` (Rebrickable `elements.csv`, `scripts/gen-ldd-element-map.py`; raw part_num, then print suffix, then assembly suffix stripped, against the UPSTREAM library), still at identity alignment, and the mini-doll slot correction then applies to the file found. Fetched only when a parse has a part it can rescue (`needsElementTable`). Counted as `elementPlacements`. On the 1,776 native `.lxf` files it rescues nothing (their 54 missing unmapped placements have no resolvable element; about 1 in 7 files carries `itemNos` at all); it helps the LXFML dumps (42703 +6 parts, 10270 +35).
