#!/usr/bin/env bun
/**
 * Grade composites for v73b buildings.
...
const gradeDir = join(tilesDir, 'grade-v73b');
...
for (const [name, coords] of Object.entries(BUILDINGS)) {
  ...
  const isoPath = join(tilesDir, 'render-v73b', `iso-${name}.jpg`);
  const topPath = join(tilesDir, 'render-v73b', `top-${name}.jpg`);
