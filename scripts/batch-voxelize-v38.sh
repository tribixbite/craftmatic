#!/bin/bash
# Batch voxelize v38: OSM mask before fill pipeline reorder
...
for name in "${BUILDINGS[@]}"; do
  ...
  $BUN scripts/voxelize-glb.ts "$glb" --auto --coords "$coords" \
    ...
    grep -E '(Grid:|Blocks|Interior|Component|Pre-fill|Mode filter|Palette:|Wrote:|Ground|Vegetation|OSM|fill)'
  echo ""
done
