#!/bin/bash
# Batch voxelize v42: geometric smoothing + relaxed window gap + partial capture guard
...
for name in "${BUILDINGS[@]}"; do
  ...
  $BUN scripts/voxelize-glb.ts "$glb" --auto --coords "$coords" \
    ...
    grep -E '(Grid:|Blocks|Interior|Component|Pre-fill|Mode filter|Palette:|Wrote:|Ground|Vegetation|OSM|fill|Window glazing|Surface smooth|Facade flat|Rect crop|Skipping rect)'
  echo ""
done
