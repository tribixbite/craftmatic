#!/bin/bash
# Batch voxelize v36: full pipeline with Gemini-reviewed fixes
# - removeSmallComponents: 500 voxel threshold (preserves multi-wing buildings)
# - glazeBackplane removed (AABB courtyard artifacts)
# - verticalRectify + horizontalRectify removed (X-only median erosion)
# - smoothRareBlocks disabled (global threshold broken after interior fill)
# - Sky-only palette remaps (dark materials preserved)
# - MBB horizontal alignment (replaces PCA diagonal)
# - Size-weighted component selection (replaces centroid proximity)
# - Curated WALL_CLUSTERS (no glass, added beige/brownstone)
...
for name in "${BUILDINGS[@]}"; do
  ...
  $BUN scripts/voxelize-glb.ts "$glb" --auto --coords "$coords" \
    ...
    grep -E '(Grid:|Blocks|Interior|Component|Pre-fill|Mode filter|Palette:|Wrote:|Ground|Vegetation|OSM)'
  echo ""
done
