#!/usr/bin/env bash
# Batch v62: v59 facade simplification (single dominant) + 12 mode-filter passes.
# Best of v59 (clean facades) + v60 (more smoothing). morphClose r=3, post-filter r=1.
set -e
...
for key in "${ORDER[@]}"; do
  ...
  schem="$OUT/${key}-v62.schem"
  echo "=== $key ==="
  $BUN "$DIR/scripts/voxelize-glb.ts" "$glb" --auto -o "$schem" 2>&1 | tail -5
  echo ""
done

echo "=== All 14 v62 done ==="

# Render all
for key in "${ORDER[@]}"; do
  schem="$OUT/${key}-v62.schem"
  jpg="$OUT/${key}-v62-iso.jpg"
  if [ -f "$schem" ]; then
    echo "Rendering $key..."
