#!/usr/bin/env bash
# Batch v61: selective facade simplification — keep top 3 types (≥5% share),
# merge minorities. Reverts morphClose to r=3, post-filter r=1. Mode filter 12 passes.
set -e
...
for key in "${ORDER[@]}"; do
  ...
  schem="$OUT/${key}-v61.schem"
  echo "=== $key ==="
  $BUN "$DIR/scripts/voxelize-glb.ts" "$glb" --auto -o "$schem" 2>&1 | tail -5
  echo ""
done

echo "=== All 14 v61 done ==="

# Render all
for key in "${ORDER[@]}"; do
  schem="$OUT/${key}-v61.schem"
  jpg="$OUT/${key}-v61-iso.jpg"
  if [ -f "$schem" ]; then
    echo "Rendering $key..."
