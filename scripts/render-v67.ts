#!/usr/bin/env bun
/**
 * Render v67 voxelized buildings as isometric exteriors.
 * Shows roof/wall material contrast from satellite-derived colors.
 */
...
const outDir = join(tilesDir, 'render-v67');
...
for (const name of BUILDINGS) {
  ...
  const schemPath = join(tilesDir, `${name}-v67.schem`);
  ...
  // Isometric exterior render (tile=4 for ARM safety)
  ...
  const tdJpg = await sharp(tdPng).jpeg({ quality: 90 }).toBuffer();
  await writeFile(join(outDir, `top-${name}.jpg`), tdJpg);
  console.log(`  → top: ${(tdJpg.length / 1024).toFixed(0)}KB`);

  if (typeof Bun !== 'undefined') Bun.gc(true);
}

console.log('\nDone! Check output/tiles/render-v67/');
