#!/bin/bash
# Batch voxelize all 12 tiles buildings — v10
...
for key in sf newton sanjose walpole byron vinalhaven suttonsbay losangeles seattle austin minneapolis charleston; do
  ...
  glb="${GLBS[$key]}"
  echo "=== $key (crop=$crop) ==="
  $BUN $SCRIPT "$DIR/$glb" $BASE --crop "$crop" -o "$DIR/${key}-v10.schem" 2>&1 | grep -E "^(OSM|Ground|Center|Grid|Wrote|Component|Veg)" || true
  echo ""
done
echo "All done."
