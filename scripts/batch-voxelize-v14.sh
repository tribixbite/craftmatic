#!/bin/bash
# Batch voxelize v14 — color pipeline fix: MIN_BRIGHT 140→60, desaturate off
# Key changes from v13: --desaturate-off preserves photogrammetry colors
set -e

BUN=~/.bun/bin/bun
DIR=output/tiles
# 2 blocks/m, crop in block units (2x meter value), 3 mode passes, 5% smooth, desaturate off
COMMON="-m surface -r 2 --generic --mode-passes 3 --smooth-pct 0.05 --desaturate-off"

vox() {
  local glb="$1" name="$2" crop="$3" coords="$4"
  echo "=== $name (crop=$crop, coords=$coords) ==="
  $BUN scripts/voxelize-glb.ts "$DIR/tiles-$glb.glb" $COMMON --crop "$crop" --coords "$coords" -o "$DIR/${name}-v14.schem" 2>&1 | grep -E 'Grid:|Palette:|Wrote:|ENU|Error|OSM footprint|crop|ground'
  echo ""
}
...
echo "Done! v14 schematics and renders in $DIR/"
