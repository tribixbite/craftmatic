...
import { renderExteriorBackLeft } from '../src/render/png-renderer.js';

const schemPath = resolve(process.argv[2]);
const outPath = process.argv[3] || schemPath.replace('.schem', '-iso-bl.jpg');
...
const pngBuf = await renderExteriorBackLeft(grid, { tile: tileSize });
