#!/usr/bin/env bun
/**
 * Render v68 voxelized buildings (1 block/ft = 3.28 blocks/m).
 * Uses tile=2 for high-res grids (~100-200 blocks per axis).
 */
...
const outDir = join(tilesDir, 'render-v68');
...
for (const name of BUILDINGS) {
  ...
  const schemPath = join(tilesDir, `${name}-v68.schem`);
  ...
  // Isometric exterior render (tile=2 for high-res grids)
  const isoPng = await renderExterior(grid, { tile: 2 });
...
console.log('\nDone! Check output/tiles/render-v68/');
