#!/usr/bin/env bash
# Batch v63: Re-captured GLBs with headless ortho camera + cylinder filter.
# Pipeline: v62 params (morphClose r=3, mode filter 12, single-dominant facade).
set -e
...
for key in "${ORDER[@]}"; do
  ...
  schem="$OUT/${key}-v63.schem"
  echo "=== $key ==="
  $BUN "$DIR/scripts/voxelize-glb.ts" "$glb" --auto -o "$schem" 2>&1 | tail -5
  echo ""
done

echo "=== All 14 v63 done ==="

# Render all
for key in "${ORDER[@]}"; do
  schem="$OUT/${key}-v63.schem"
  jpg="$OUT/${key}-v63-iso.jpg"
  if [ -f "$schem" ]; then
    echo "Rendering $key..."
