...
for (const f of files) {
  ...
  for (const p of audit.parts) {
    ...
    if (all || bad || stuck || !p.reachable) console.log(`${f.split(/[\/]/).pop()!.padEnd(44)} ${p.label.padEnd(20)} reachable ${String(p.reachable).padStart(4)}  accepted ${String(p.accepted).padStart(4)}  closes ${String(p.closable).padStart(4)}  boxes ${p.boxes} (unreached ${p.unreachedBoxes})${bad ? '  REFUSED' : stuck ? '  OPENS ONLY' : !p.reachable ? '  OUT OF REACH' : ''}`);
...
console.log(`${files.length} packs, ${total} parts: ${refused} refuse every tap, ${oneWay} open but close again from no spot that opened them, ${unreachable} out of reach from any standing spot`);
