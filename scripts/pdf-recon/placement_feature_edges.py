...
class DepthRecordingRasterizer(LayerRasterizer):
    def render(self,triangles,colors,width,height):
        self.last=self.render_depth(triangles,colors,width,height)
        return self.last['rgb'],self.last['mask']
...
class FeatureEdgeScorer(GpuColoredSceneScorer):
    def __init__(self,*args,**kwargs):
        ...
        if self.plane_depth:self.rasterizer=DepthRecordingRasterizer()
        self.last_layer=None;self.last_layer_key=None
    def score(self,items,projection,renderpath=None):
        ...
        for part,color,T in items:
            ...
            if 'feature_normals' not in p:
                ...
                p['feature_normals']=n
            normals.append(p['feature_normals'])
        layer=self.rasterizer.last if self.plane_depth else self.layer_rasterizer.render_depth(np.concatenate(triangles)[None],np.concatenate(colors)[None],self.rgb.shape[1],self.rgb.shape[0])
        self.last_layer=layer
        self.last_layer_key=(tuple((p,str(c),np.asarray(T,float).tobytes()) for p,c,T in items),M.tobytes())
