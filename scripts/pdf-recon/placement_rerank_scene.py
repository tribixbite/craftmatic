...
def run(pdf,source,out,edge_weight=.5):
    ...
    scorer=PdfFeatureSceneScorer(scene,plane_depth=True,edge_weight=edge_weight)
    ...
    report=dict(pdf=str(pdf.resolve()),pdf_sha256=digest,pdf_only=True,truth_used=False,runtime_vlm_calls=0,
        ...
        selected_parts=len(candidates[0]['items']),edge_weight=edge_weight,results=records,
...
if __name__=='__main__':
    ...
    a=p.parse_args();run(a.pdf,a.source,a.out,a.edge_weight)
