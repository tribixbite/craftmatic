#!/usr/bin/env bun
/**
 * Render v70 voxelized buildings — isometric + top-down views.
 */
...
const outDir = join(tilesDir, 'render-v70');
await mkdir(outDir, { recursive: true });

const BUILDINGS = ['green', 'dakota', 'noe', 'sentinel', 'francisco', 'beach', 'chestnut'];
const nameArg = process.argv.find(a => a.startsWith('--name='));
const filterName = nameArg ? nameArg.split('=')[1] : null;

for (const name of BUILDINGS) {
  if (filterName && name !== filterName) continue;
  const schemPath = join(tilesDir, `${name}-v70.schem`);
