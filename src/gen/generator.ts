...
export function generateStructure(options: GenerationOptions): BlockGrid {
  ...
  switch (type) {
    ...
    case 'house':
    default:
      grid = generateHouse(floors, style, rooms, width, length, rng, options.roofShape, options.features, options.floorPlanShape, options.roofHeightOverride, options.windowSpacing);
      break;
  }
