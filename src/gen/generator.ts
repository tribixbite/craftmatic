...
function compoundify(
  ...
  if (type === 'house') {
      ...
      for (let z = bzMid - 7; z < bzMid + 7 + 6; z++) {
        for (let y = 3; y < gh; y++) {
          if (compound.inBounds(x, y, z) && compound.get(x, y, z) !== 'minecraft:air') {
            ...
            else if (rng() < 0.3) compound.set(x, y, z, 'minecraft:mossy_stone_bricks');
          }
        }
      }
    }
    // East: graveyard with watchtower
    const wtX = bx2 + 4;
    const wtZ = bzMid - 2;
    // Simple watchtower (4x4, 8 tall, open top)
    for (let y = 0; y <= 8; y++) {
    ...
    // Crenellations on watchtower
    for (let dx = 0; dx < 4; dx++) {
      for (let dz = 0; dz < 4; dz++) {
        if ((dx === 0 || dx === 3 || dz === 0 || dz === 3) && (dx + dz) % 2 === 0) {
          if (compound.inBounds(wtX + dx, 9, wtZ + dz))
            compound.set(wtX + dx, 9, wtZ + dz, 'minecraft:mossy_stone_bricks');
        }
      }
    }
    // Gravestones around watchtower
    for (let i = 0; i < 8; i++) {
      const gx = wtX - 2 + Math.floor(rng() * 8);
      const gz = wtZ - 2 + Math.floor(rng() * 8);
      if (compound.inBounds(gx, 1, gz) && compound.get(gx, 1, gz) === 'minecraft:air')
